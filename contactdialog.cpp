...
#include <QToolTip>
#include <QMenu>        // ДОБАВИТЬ
#include <QAction>      // ДОБАВИТЬ
...
Contact ContactDialog::getContact() const {
    ...
    // НОРМАЛИЗУЕМ имена перед сохранением
    ...
    for (int i = 0; i < ui->phoneListWidget->count(); ++i) {
        ...
        if (match.hasMatch()) {
            ...
            if (typeStr == "Work") type = PhoneType::WORK;
            else if (typeStr == "Office") type = PhoneType::OFFICE;
...
void ContactDialog::editPhoneItem(int row) {
    ...
    if (match.hasMatch()) {
        ...
        // Загружаем в поля редактирования
        QString normalized = QtValidator::normalizePhone(phoneNumber);
